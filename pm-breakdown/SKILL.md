...
nanopm_preamble
_TASKS_DIR=".nanopm/wiki/docs/tasks"
mkdir -p "$_TASKS_DIR"
...
```bash
ls .nanopm/wiki/docs/prds/*.md 2>/dev/null || echo "NO_PRDS"
```

**If NO_PRDS:** "No PRDs found in `.nanopm/wiki/docs/prds/`. Run `/pm-prd` first to create one." Exit.
...
- **Source PRD:** `.nanopm/wiki/docs/prds/{_FEATURE_SLUG}.md` (read this first — it's the source of truth)
...
1. Read `.nanopm/wiki/docs/prds/{_FEATURE_SLUG}.md` carefully. The PRD is the source of truth. If anything here conflicts with the PRD, the PRD wins.
...
*This WORKFLOW.md was generated by nanopm /pm-breakdown --target=symphony on {date}. The PRD at `.nanopm/wiki/docs/prds/{_FEATURE_SLUG}.md` is the canonical specification. Symphony spec compatibility: v1 (Linear tracker, repo-owned WORKFLOW.md).*
```
...
- A back-link to the PRD: `Source PRD: .nanopm/wiki/docs/prds/{_FEATURE_SLUG}.md`
...
Regardless of target, write `.nanopm/wiki/docs/tasks/{_FEATURE_SLUG}.md`:
...
- **Tasks:** {N} ({sum} effort) → [`.nanopm/wiki/docs/tasks/{_FEATURE_SLUG}.md`](.nanopm/wiki/docs/tasks/{_FEATURE_SLUG}.md)
...
Tell the user:
- Tasks file: `.nanopm/wiki/docs/tasks/{_FEATURE_SLUG}.md` (includes the Build Plan + GUI tests)
