...
11. When competitor intel exists (`competitors.json` + `intel/`), the nav shows a Competitors section: report rows (latest + dated history) and one row per competitor opening a page with its monitored-page links, last-checked time, and per-page snapshots as tabs. Competitor-owned files are grouped there instead of cluttering the phase lists.
