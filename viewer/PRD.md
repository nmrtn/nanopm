...
- **Launching NanoPM runs from the app** — deferred per the scope decision; this v1 is a viewer, not an orchestrator. Revisit only on a positive prototype read (STRATEGY.md no-go: "no building the full app before the prototype reads positive").
...
7. The app exposes no write, rename, or delete operation on `.nanopm/` contents.
