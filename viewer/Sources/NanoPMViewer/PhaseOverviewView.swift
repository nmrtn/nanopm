...
struct PhaseOverviewView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                if phase == .define {
                    briefCard(file: "CONTEXT-SUMMARY.md",
                              title: "Context Brief",
                ...
                if phase == .plan {
                    briefCard(file: "PLAN-SUMMARY.md",
                              title: "Plan Brief",
    ...
    /// (CONTEXT-SUMMARY.md) on Define, the Plan Brief (PLAN-SUMMARY.md) on Plan. Both
    /// are regenerated by a subagent after their phase's skills run and reloaded into
    /// every skill's preamble, so this card shows the same context the agent carries.
    /// One parameterized card: only one phase shows per view, so a single briefContent
    /// state is safe.
    @ViewBuilder
    private func briefCard(file: String, title: String, loadedSubtitle: String,
        ...
        .task(id: "\(brief?.id ?? "none")#\(store.generation)") { await loadBrief(file: file) }
    }
